		...
		const AGLHandler = ModuleHandler.extend({
			...
			onElementChange: function (changedProp) {
				...
				if (settings['agl_in_name']) {
					let newClasses = ` agl agl-${settings['agl_in_name']}${settings['agl_in_direction']} agl-in-duration-${settings['agl_in_duration'].size} agl-in-delay-${settings['agl_in_delay'].size}`
					if(settings['agl_in_easing'])
						newClasses += ` agl-in-easing-${settings['agl_in_easing']}`
